...
function validerBildenr($bildenr)
{
  ...
  else if(!ctype_upper(substr($bildenr, 0, -1)))
  {
    $lovligBildenr=false;
...
function validerOpplastingsdato($opplastingsdato)
{
  $lovligOpplastingsdato=true;
  if(!$opplastingsdato)
  {
    $lovligOpplastingsdato=false;
    print ("Opplastingsdato må fylles ut!!");
  }
  else if(strlen($opplastingsdato)<10)
  {
    $lovligOpplastingsdato=false;
    print ("Opplastingsdato skal ha 8 tall og to bindestreker - totalt 10 tegn!");
  }
  else if(!ctype_upper(substr($opplastingsdato, 0, -1)))
  {
    $lovligOpplastingsdato=false;
    print ("Opplastingsdato skal ha 10 tegn.");
  }
  else if(!is_numeric(substr($opplastingsdato, -1)))
  {
    $lovligOpplastingsdato=false;
    ("Det siste symboler er ikke et siffer!!");
  }
